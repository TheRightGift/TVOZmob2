...
const database_name = "ozTvMedia.db";
const database_version = "1.0";
const database_displayname = "OZTV database";
...
export default class Database {
			...
			SQLite.openDatabase(
				...
				.then(DB => {
					...
					db.executeSql('SELECT 1 FROM User LIMIT 1').then(() => {
						...
						db.transaction((tx) => {
							tx.executeSql('CREATE TABLE IF NOT EXISTS User (id Text Not Null, firstname TEXT NOT NULL, surname TEXT NOT NULL, othername TEXT NOT NULL, username varchar(20) NOT NULL, password varchar(20) NOT NULL, dateJoined date NOT NULL, phone varchar(11) NOT NULL, address TEXT NOT NULL, macAddress varchar(20) NOT NULL)');
							...
							tx.executeSql('CREATE TABLE IF NOT EXISTS payment (id TEXT NOT NULL, digit INTEGER NOT NULL, dateUsed Date NOT NULL, dateExpire Date NOT NULL, numOfDays INTEGER NOT NULL)');
						}).then(() => {
	...
	addUser(user) {
		return new Promise((resolve) => {
			this.initDB().then((db) => {
				db.transaction((tx) => {					
					tx.executeSql('INSERT INTO User (id, firstname, surname, othername, username, password, dateJoined, phone, address, macAddress) VALUES ("'+user.id+'", "'+user.fName+'", "'+user.lName+'", "'+user.oName+'", "'+user.uName+'", "'+user.pass+'", "'+user.date+'", "'+user.phone+'", "'+user.add+'", "'+user.mac+'")', []).then(([tx, results]) => {
						resolve(results);
					});
				}).then((result) => {
					//this.closeDatabase(db);
				...
				db.transaction((tx) => {
					tx.executeSql('SELECT * FROM user', []).then(([tx, results]) => {
						...
						for (let i = 0; i < len; i++) {
							...
							const { id, firstname, surname, othername, username, password, dateJoined, phone, address  } = row;
							uData.push({
								id, 
								firstname, 
								surname, 
		...
		return new Promise((resolve) => {
			this.initDB().then((db) => {
				db.transaction((tx) => {
					...
					tx.executeSql('DELETE FROM payment', []).then(([tx, results]) => {
						resolve(results);
					});
				}).then((result) => {
					//this.closeDatabase(db);
	...
	addPayment(sub) {
		...
		return new Promise((resolve) => {
			this.initDB().then((db) => {
				db.transaction((tx) => {
					tx.executeSql('INSERT INTO payment VALUES (?, ?, ?, ?, ?)', [sub.id, digit, sub.dateUsed, sub.dateExpire, sub.numOfDays]).then(([tx, results]) => {
						resolve(results);
					});
				}).then((result) => {
					//this.closeDatabase(db);
				...
				db.transaction((tx) => {
					tx.executeSql("SELECT * FROM payment ORDER BY dateUsed DESC", []).then(([tx,results]) => {
						...
						for (let i = 0; i < len; i++) {
							...
							const { id, digit, dateUsed, dateExpire, numOfDays } = row;
							payHistory.push({
								id,
								digit,
								dateUsed,
	...
	lastPaymentHistory() {
		...
		return new Promise((resolve) => {
			...
			this.initDB().then((db) => {
				db.transaction((tx) => {
					tx.executeSql("SELECT * FROM payment ORDER BY dateUsed DESC LIMIT 1", []).then(([tx,results]) => {
						var len = results.rows.length;
